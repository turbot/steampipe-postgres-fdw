...
PG_VERSION="PostgreSQL 14.9 (Homebrew)
"
...
confirm_and_install() {
  ...
  if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ] || [ -z "$REPLY" ]; then
    ...
    cp steampipe_postgres_trivy.so "$LIBDIR"
    cp steampipe_postgres_trivy.control "$EXTDIR"
    cp steampipe_postgres_trivy--1.0.sql "$EXTDIR"

    # Check if the files were copied correctly
    if [ $? -eq 0 ]; then
      echo ""
      echo "Successfully installed steampipe_postgres_trivy extension!"
      echo ""
      echo "Files have been copied to:"
      echo "- Library directory: ${LIBDIR}"
      echo "- Extension directory: ${EXTDIR}"
    else
      echo "Failed to install steampipe_postgres_trivy extension. Please check permissions and try again."
      exit 1
    fi
    exit 0
  else
